...
public class Deck {
    ...
    public void shuffle() {
        ...
        shuffled[0] = cards.get(intRandom);
        cards.set(intRandom, null);
        
        for(int intI = 1; intI < cards.size(); intI++) {       
            intRandom = (int) (Math.random() * (cards.size()));
            while(cards.get(intRandom) == null)
                intRandom = (int)(Math.random() * (cards.size()));
            shuffled[intI] = cards.get(intRandom);
            cards.set(intRandom, null);
        }
        
        for(int intI = 0; intI < cards.size(); intI++) {
           cards.set(intI, shuffled[intI]); 
        }      
        size = cards.size();
    }
